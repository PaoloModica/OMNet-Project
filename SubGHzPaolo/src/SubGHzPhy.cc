...
simsignal_t SubGHzPhy::bitrateSignal = SIMSIGNAL_NULL;
simsignal_t SubGHzPhy::radioStateSignal = SIMSIGNAL_NULL;
simsignal_t SubGHzPhy::channelNumberSignal = SIMSIGNAL_NULL;
simsignal_t SubGHzPhy::lossRateSignal = SIMSIGNAL_NULL;

simsignal_t SubGHzPhy::collisionSignal = SIMSIGNAL_NULL;


uint16_t SubGHzPhy::aMaxPHYPacketSize  = 127;      //max PSDU size (in bytes) the PHY shall be able to receive
uint16_t SubGHzPhy::aTurnaroundTime    = 12;       //Rx-to-Tx or Tx-to-Rx max turnaround time (in symbol period)
uint16_t SubGHzPhy::aMaxBeaconOverhead             = 75;       //max # of octets added by the MAC sublayer to the payload of its beacon frame
uint16_t SubGHzPhy::aMaxBeaconPayloadLength = aMaxPHYPacketSize - aMaxBeaconOverhead;       //max size, in octets, of a beacon payload
uint16_t SubGHzPhy::aMaxFrameOverhead              = 25;       //max # of octets added by the MAC sublayer to its payload w/o security.//max # of octets that can be transmitted in the MAC frame payload field
uint16_t SubGHzPhy::aMaxMACFrameSize  = aMaxPHYPacketSize - aMaxFrameOverhead;
simsignal_t SubGHzPhy::changeLevelNoise80215 = SIMSIGNAL_NULL;

#define BASE_NOISE_LEVEL (noiseGenerator?noiseLevel+noiseGenerator->noiseLevel():noiseLevel)


SubGHzPhy::SubGHzPhy() : rs(this->getId())
{
    //radio2 = NULL;      //questa
    radioMod = NULL;
    receptionModel = NULL;
...
void SubGHzPhy::registerBattery()
{
...
void SubGHzPhy::initialize(int stage)
{
    ChannelAccess::initialize(stage);
    datarate = par("datarate");
    hl = par("hl");
    collisionNum = par("collisionNum");
    EV << getParentModule()->getParentModule()->getFullName() << ": initializing SubGHzPhy, stage=" << stage << endl;
    ...
    if (stage == 0)
    {
        ...
        rs.setBitrate(datarate);                    //modifica - come da progetto - Elaborato 1
...
bool SubGHzPhy::processAirFrame(AirFrame *airframe)
{
...
void SubGHzPhy::handleMessage(cMessage *msg)
{
    // handle primitives
...
void SubGHzPhy::handlePrimitive(int msgkind, cMessage *msg)
{
...
AirFrame* SubGHzPhy::encapsulatePacket(cMessage *frame)
{
    ...
    airframe->setDuration(radioMod->calculateDuration(airframe));
...
void SubGHzPhy::handleUpperMsg(AirFrame *airframe)
{
    if (phyRadioState == phy_TX_ON)
    {
        ...
        emit(bitrateSignal, rs.getBitrate());           //modifica
...
void SubGHzPhy::sendUp(cMessage *msg)
{
    if (receiverConnect)
    {
...
void SubGHzPhy::sendDown(AirFrame *airframe)
{
    sendToChannel(airframe);
}

void SubGHzPhy::bufferMsg(AirFrame *airframe) //FIXME: add explicit simtime_t atTime arg?
{
...
AirFrame* SubGHzPhy::unbufferMsg(cMessage *msg)
{
...
void SubGHzPhy::handleLowerMsgStart(AirFrame * airframe)
{
...
void SubGHzPhy::handleLowerMsgEnd(AirFrame * airframe)
{
    ...
    if (snrInfo.ptr == airframe)
    {
        ...
        {
            EV << "[PHY]: reception of r/ieee802154/SubGHzPhy.cc line 432." << airframe->getName() << " frame failed because MAC layer turned off the receiver forcibly during the reception, drop it \n";
        ...
        else
        {
            ...
            PhyIndication frameInfo = radioMod->isReceivedCorrectly(airframe, list);
            if (frameInfo != FRAMEOK)
            {
                isCollision = true;
                collisionNum++;                             //modifica - come da progetto - Elaborato 1
                emit(collisionSignal, collisionNum);
    ...
    else
    {
        ...
        lossRate = (double)numGivenUp/((double)numReceivedCorrectly+(double)numGivenUp);
        emit(lossRateSignal, lossRate);
        //a qui, aggiunto per gestire la ricezione di una airframe troppo disturbata - come da progetto - Elaborato 1
...
void SubGHzPhy::handleSelfMsg(cMessage *msg)
{
    switch (msg->getKind())
    {
...
void SubGHzPhy::PD_DATA_confirm(PHYenum status)
{
...
void SubGHzPhy::PD_DATA_confirm(PHYenum status,short additional)
{
    Ieee802154MacPhyPrimitives *primitive = new Ieee802154MacPhyPrimitives();
    primitive->setKind(PD_DATA_CONFIRM);
    primitive->setStatus(status);
...
void SubGHzPhy::PLME_CCA_confirm(PHYenum status)
{
    Ieee802154MacPhyPrimitives *primitive = new Ieee802154MacPhyPrimitives();
...
void SubGHzPhy::PLME_bitRate(double bitRate)
{
    Ieee802154MacPhyPrimitives *primitive = new Ieee802154MacPhyPrimitives();
...
void SubGHzPhy::PLME_ED_confirm(PHYenum status, uint16_t energyLevel)
{
    Ieee802154MacPhyPrimitives *primitive = new Ieee802154MacPhyPrimitives();
...
void SubGHzPhy::PLME_SET_TRX_STATE_confirm(PHYenum status)
{
    Ieee802154MacPhyPrimitives *primitive = new Ieee802154MacPhyPrimitives();
...
void SubGHzPhy::PLME_SET_confirm(PHYenum status, PHYPIBenum attribute)
{
    Ieee802154MacPhyPrimitives *primitive = new Ieee802154MacPhyPrimitives();
...
void SubGHzPhy::handle_PLME_SET_TRX_STATE_request(PHYenum setState)
{
    bool delay;
    PHYenum tmp_state;
...
void SubGHzPhy::handle_PLME_SET_request(Ieee802154MacPhyPrimitives *primitive)
{
...
void SubGHzPhy::changeChannel(int channel)
{
   ...
   for (RecvBuff::iterator it = recvBuff.begin(); it!=recvBuff.end(); ++it)
   {
    ...
    rs.setBitrate(datarate);        //modifica
    emit(bitrateSignal, rs.getBitrate());
...
bool SubGHzPhy::channelSupported(int channel)
{
    return ((def_phyChannelsSupported & (1 << channel)) != 0);
}

double SubGHzPhy::getRate(char bitOrSymbol)
{
   double rate=datarate;
...
uint16_t SubGHzPhy::calculateEnergyLevel()
{
    int energy;
...
void SubGHzPhy::setRadioState(RadioState::State newState)
{
...
void SubGHzPhy::updateDisplayString() {
...
double SubGHzPhy::calcDistFreeSpace()
{
...
void SubGHzPhy::disconnectReceiver()
{
...
void SubGHzPhy::connectReceiver()
{
...
bool SubGHzPhy::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void SubGHzPhy::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj)
{
